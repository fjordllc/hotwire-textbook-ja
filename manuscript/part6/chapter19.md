...
この書き方には、Turbo と相性が悪い問題があります。`DOMContentLoaded` は、最初のページ読み込みのときだけ発火します。Turbo はページ遷移で `<body>` を差し替えますが（第7章）、そのときこのイベントは発火しません。結果、Turbo で遷移した先では、結びつけたはずの振る舞いが動かなくなります。
...
- `controllers/users/list_controller.js` → `data-controller="users--list"`（サブディレクトリは `--` で区切る）

`_controller.js` の部分は付けず、それより前の部分が名前になります。
