...
frame には、はっきりした限界があります。<strong>1 回の frame navigation で Turbo が差し替える対象は、1 つの frame だけ</strong>です（frame の中にある複数の要素は、その frame ごとまとめて差し替わります）。
...
![独立した部分更新の要否と更新箇所数で、通常遷移・Frame・Streams を選ぶフローチャート。](../figures/fig-14-1.svg)
