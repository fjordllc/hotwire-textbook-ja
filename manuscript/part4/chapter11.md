...
もし、リンク先に同じ `id` の frame がなかったらどうなるでしょうか。その場合、frame の中身は空になり、ブラウザの Console にエラーが出ます。これは Turbo Frames でいちばん多いつまずきです。「frame が空になった」「Content missing と出る」ときは、まず<strong>両側の `id` が一致しているか</strong>を確認してください（よくあるエラーは付録Eにまとめます）。
