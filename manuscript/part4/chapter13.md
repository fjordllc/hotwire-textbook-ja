...
frame に `src` 属性を与えると、frame は表示されたときに、その URL から中身を取得します。
...
`loading: :lazy` を付けると、frame が画面に見えるようになったタイミングで読み込みます。`src` 先のレスポンスには、第11章のルールどおり、同じ `id="project_tasks"` の frame が必要です。`tasks_panel.html.erb` がそれを満たしています。
...
frame は、`src` 先を読み込み直すこともあります。たとえば、内容が変わったときに frame を再読み込みする場合です。

このとき、既定では frame の中身がまるごと差し替わります。差し替えのたびに、frame の中のスクロール位置や入力中のフォーカスが失われます。

これを避けたいときに、`refresh="morph"` を使います。
...
`refresh: :morph` を付けると、frame の再読み込みが、第9章で見た morph（差分の適用）で行われます。変わった部分だけが書き換わり、frame の中の状態が保たれます。frame を繰り返し更新する画面で効きます。これは、第18章のリアルタイム更新とも組み合わせられます。
