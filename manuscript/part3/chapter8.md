...
![フォーム送信の契約。成功=redirect（update/destroy は 303）、失敗=422 render、200 は差し替わらず送信元 URL に留まる、という分岐を示すフローチャート](../figures/fig-8-1.svg)
...
この契約は、[第25章](../part7/chapter25.md)「バリデーションエラーとフォーム UX」で本格的に使います。ここでは「成功は redirect、失敗は 422」という対応を覚えておいてください。
...
> 第8章では、フォーム送信も visit であり、成功は redirect、失敗は 422 という契約があることを見ました。次の [第9章](chapter9.md) では、visit を速く見せるキャッシュと、Turbo 8 の morphing を扱います。
