...
![リンククリックから fetch、HTML 応答、body 差し替えまでの流れを示すシーケンス図。](../figures/fig-7-2.svg)
...
![Turbo Drive のページ置換。通常遷移は全体を作り直すが、visit は body だけを差し替える（head はマージ）](../figures/fig-7-1.svg)
...
> 第7章では、Turbo Drive のリンク遷移を visit として理解しました。次の [第8章](chapter8.md) では、フォーム送信も同じ visit であること、そして成功と失敗で controller が返すべきものが決まっていることを見ます。
