...
<!-- fig-2-1: 画面更新の歴史。全ページ再読み込み→Ajax→JSON/SPA→HTML over the wire の流れと、各段の利点と代償を並べたタイムライン -->

<!-- fig-2-2: JSON+SPA と HTML over the wire の通信比較。wire を流れるもの（JSON か HTML か）と画面を組み立てる場所（ブラウザかサーバーか）の違いを対比 -->
