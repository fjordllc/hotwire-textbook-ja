...
- 半角英数字・英単語と日本語の間には、半角スペースを 1 つ入れます。
- このルールは本文だけでなく、見出し、キャプション、`manuscript/SUMMARY.md` の目次表記にも適用します。
- `Hotwireは` ではなく `Hotwire は`、`Turbo Driveという` ではなく `Turbo Drive という` とします。
- ただし `第8章` `第1部` `付録A` のような日本語の番号ラベル内では空けません。`第 8 章` `第 1 部` `付録 A` にはしません。
...
- 本文中で別の章や付録を参照するときは、必要に応じて Markdown の相対リンクを使います。例: `[第8章](../part3/chapter8.md)`、`[付録E](appendix/e.md)`。
- 章や付録への導線は、読み進める助けになる箇所だけをリンクにします。すべての出現箇所をリンク化する必要はありません。
- 章末の接続文、導入の「この章のねらい」、付録の案内、用語の初出は、リンク化の優先度が高いです。
- ページ内の目次や長い文書内のジャンプには、明示的な `id` と `[#anchor](#anchor)` を使います。`<a id="..."></a>` とセットで書きます。
- リンクテキストは、遷移先が分かる短い文言にします。`こちら` のような曖昧な表現は避けます。
- 章番号や部番号は、リンク先が一意に分かるなら本文中でそのまま使えますが、読者が戻れる導線として有益な場合はリンクにします。
